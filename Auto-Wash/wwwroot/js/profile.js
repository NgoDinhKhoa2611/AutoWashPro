    ...
    if (typeof isGoogleAccount !== 'undefined' && isGoogleAccount) {
        ...
        try {
            ...
            if (response.ok && data.success) {
                ...
                let desc = `Hệ thống đã gửi một mã xác thực OTP 6 chữ số tới email đăng ký: <span class="text-cyan fw-bold">${userEmail}</span>. Mã có giá trị trong 5 phút.`;
                // Extract simulated code to display on UI if in testing/mock mode
                if (data.message && data.message.includes('Mã mô phỏng:')) {
                    const match = data.message.match(/Mã mô phỏng:\s*(\d+)/);
                    if (match && match[1]) {
                        desc += `<br/><br/><span class="badge bg-warning text-dark p-2" style="font-size:0.85rem; border-radius:8px;"><i class="fas fa-flask me-1"></i> [Mock Test] Mã OTP: <strong>${match[1]}</strong></span>`;
                    }
                }
                document.getElementById('profile-otp-description').innerHTML = desc;
        ...
        if (!userPhone || !userPhone.trim()) {
            showToast('Vui lòng cập nhật số điện thoại trong hồ sơ trước khi thực hiện thay đổi mật khẩu!', 'warning');
            return;
        }

        // Sanitize and format the phone number to E.164 (+84...) for Firebase SMS Auth
        ...
        if (isFirebaseMockMode || !firebaseAuth) {
            setTimeout(() => {
                ...
                document.getElementById('profile-otp-description').innerHTML = `Hệ thống đã gửi mã xác thực OTP 6 chữ số tới số điện thoại đăng ký: <span class="text-cyan fw-bold">${userPhone}</span> qua Firebase.<br/><br/><span class="badge bg-warning text-dark p-2" style="font-size:0.85rem; border-radius:8px;"><i class="fas fa-flask me-1"></i> [Mock Test] Mã OTP Sandbox: <strong>123456</strong></span>`;
        ...
        try {
            ...
            document.getElementById('profile-otp-description').innerHTML = `Hệ thống đã gửi mã xác thực OTP 6 chữ số tới số điện thoại đăng ký: <span class="text-cyan fw-bold">${userPhone}</span> (định dạng: <span class="text-cyan fw-bold">${formattedPhone}</span>) qua Firebase.`;
            ...
            document.getElementById('profile-otp-description').innerHTML = `Hệ thống đã gửi mã xác thực OTP 6 chữ số tới số điện thoại đăng ký: <span class="text-cyan fw-bold">${userPhone}</span> (Mô phỏng Sandbox).<br/><br/><span class="badge bg-warning text-dark p-2" style="font-size:0.85rem; border-radius:8px;"><i class="fas fa-flask me-1"></i> [Mock Test] Mã OTP Sandbox: <strong>123456</strong></span>`;
            const modal = new bootstrap.Modal(document.getElementById('otpChangePasswordModal'));
            modal.show();
            startProfileOtpTimer();
        }
...
// ── Vehicles (C# EF Core Backend Integration - Rule A-6) ──
let vehicleOtpTimer = null;
let pendingVehiclePlate = '';
let pendingVehicleType = '';
...
async function renderVehicles() {
    ...
    try {
        const response = await fetch('/Customer/GetVehicles');
        const data = await response.json();
...
async function handleDeleteVehicle(plate) {
    const confirmFn = async () => {
        try {
            const response = await fetch('/Customer/DeleteVehicle', {
                method: 'POST',
            ...
            if (response.ok && data.success) {
                ...
                showToast(data.message || 'Xoá phương tiện thất bại!', 'error');
            }
        } catch (err) {
...
async function handleRequestVehicleOtp() {
    let plate = getVal('new-plate').trim().toUpperCase().replace(/[-\s.]/g, '');
    const type  = getVal('new-type');

    if (!plate) { showToast('Vui lòng nhập biển số xe!', 'warning'); return; }

    // Validate XXA12345 format (exactly 3 prefix alphanumeric characters followed by 5 digits)
    const match = plate.match(/^([A-Z0-9]{3})(\d{5})$/);
    if (!match) {
        showToast('Biển số xe không hợp lệ! Vui lòng nhập đúng định dạng 8 ký tự (ví dụ: 51H12345).', 'warning');
        return;
    }

    const prefix = match[1];
    const provinceCode = prefix.substring(0, 2);
    ...
    if (!validProvinces.includes(provinceCode)) {
        showToast('Đầu số tỉnh thành không hợp lệ! Vui lòng nhập đúng đầu số Việt Nam (VD: 51, 59, 67, 68, 18, 80,...)', 'warning');
        return;
    }

    const digits = match[2];
    const formattedPlate = `${prefix} - ${digits.substring(0, 3)}.${digits.substring(3)}`;

    const requestBtn = document.getElementById('btn-request-vehicle-otp');
    if (requestBtn) requestBtn.disabled = true;
    showToast('Đang tạo mã xác thực đăng ký xe...', 'info');

    try {
        const response = await fetch('/Customer/SendVehicleOtp', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ LicensePlate: formattedPlate })
        });

        const data = await response.json();

        if (response.ok && data.success) {
            pendingVehiclePlate = formattedPlate;
            pendingVehicleType = type;

            showToast('Đã gửi mã xác thực OTP đăng ký xe!', 'success');
            
            // Switch modal step
            document.getElementById('add-vehicle-step-input').style.display = 'none';
            document.getElementById('add-vehicle-step-otp').style.display = 'block';
            document.getElementById('add-vehicle-modal-title').innerHTML = '<i class="fas fa-shield-alt me-2 text-cyan"></i>XÁC THỰC OTP ĐĂNG KÝ XE';
            
            // Display only standard secure text (no mock/sandbox OTP code shown on UI)
            let desc = `Hệ thống đã gửi một mã xác thực OTP 6 chữ số tới email đăng ký của bạn: <span class="text-cyan fw-bold">${userEmail}</span>. Mã có giá trị trong 5 phút.`;
            document.getElementById('vehicle-otp-description').innerHTML = desc;

            startVehicleOtpTimer();
        } else {
            showToast(data.message || 'Lỗi khi yêu cầu mã xác thực xe!', 'error');
        }
    } catch (err) {
        console.error("Error requesting vehicle OTP:", err);
        showToast('Lỗi kết nối máy chủ gửi OTP!', 'error');
    } finally {
        if (requestBtn) requestBtn.disabled = false;
    }
}

async function handleVerifyVehicleOtpAndSave() {
    const digits = [0, 1, 2, 3, 4, 5].map(i => {
        const el = document.getElementById('veh-otp-' + i);
        return el ? el.value : '';
    });
    const code = digits.join('');
    ...
    const verifyBtn = document.getElementById('btn-verify-vehicle-otp');
    if (verifyBtn) verifyBtn.disabled = true;

    try {
        const response = await fetch('/Customer/VerifyVehicleOtpAndSave', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                LicensePlate: pendingVehiclePlate,
                Type: pendingVehicleType,
                OtpCode: code
            })
        });
        ...
        if (response.ok && data.success) {
            clearInterval(vehicleOtpTimer);
            showToast('Đăng ký phương tiện mới thành công!', 'success');
        ...
        console.error("Error verifying vehicle OTP:", err);
        showToast('Không thể kết nối với máy chủ xác minh!', 'error');
    } finally {
        if (verifyBtn) verifyBtn.disabled = false;
    }
...
function startVehicleOtpTimer() {
    let seconds = 300;
    const timerEl = document.getElementById('vehicle-otp-timer');
    const resendMsg = document.getElementById('vehicle-otp-resend-wrapper');
    const resendBtn = document.getElementById('vehicle-otp-resend-btn');

    if (resendMsg) resendMsg.style.display = 'block';
    if (resendBtn) resendBtn.style.display = 'none';

    clearInterval(vehicleOtpTimer);
    vehicleOtpTimer = setInterval(() => {
        seconds--;
        if (timerEl) {
            let mins = Math.floor(seconds / 60);
            let secs = seconds % 60;
...
function resetVehicleModal() {
    clearInterval(vehicleOtpTimer);
    document.getElementById('add-vehicle-step-input').style.display = 'block';
    document.getElementById('add-vehicle-step-otp').style.display = 'none';
    document.getElementById('add-vehicle-modal-title').innerHTML = '<i class="fas fa-motorcycle me-2 text-cyan"></i>THÊM PHƯƠNG TIỆN MỚI';
    setVal('new-plate', '');
...
function handleVehOtpInput(idx, el) {
    el.value = el.value.replace(/\D/, '');
    if (el.value.length === 1 && idx < 5) {
        const next = document.getElementById('veh-otp-' + (idx + 1));
        if (next) next.focus();
    }
    el.classList.toggle('filled', el.value.length > 0);
}

function handleVehOtpKeydown(idx, e) {
    if (e.key === 'Backspace') {
        const el = document.getElementById('veh-otp-' + idx);
        if (el && el.value === '' && idx > 0) {
            const prev = document.getElementById('veh-otp-' + (idx - 1));
            if (prev) { prev.value = ''; prev.classList.remove('filled'); prev.focus(); }
        }
    }
}
